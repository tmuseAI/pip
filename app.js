    ...
    function paintBlobs(time) {
      ...
      blobs.forEach((b) => {
        const x =
          w *
          (b.ox +
            b.sx * 0.065 * Math.sin(time * b.px + b.oy * 6));
        const y =
          h *
          (b.oy +
            b.sy * 0.055 * Math.cos(time * b.py + b.ox * 5));
        const rad =
          Math.max(w, h) * b.r * (1 + 0.035 * Math.sin(time * 0.72 + b.ox * 4));
  ...
  function initUiScale() {
    ...
    try {
      ...
      if (saved === "2x" || saved === "3x") {
        initial = "1x";
        localStorage.setItem(MODE_KEY, "1x");
    ...
    buttons.forEach((btn) => {
      btn.addEventListener("click", () => {
        setActive(btn.getAttribute("data-ui-mode"));
      });
    });
  }
